...
function InsertOrder(orderid,ordertime,name){
	var sql = `INSERT INTO o_order(orderid,ordertime,orderman,state) VALUES ('${orderid}','${ordertime}','${name}','0')`
	con.query(sql,(err,results,fields) => {
	  if (err) {
...
router.post('/api/order',function(req,res){
	...
	var id
	//先判断是否已有该订单号
	// var sql_3 = `SELECT orderid FROM o_order WHERE orderman = '${name}' AND state = '0'` 
	var search = new Promise(function(resolve,reject){ 
		var sql_3 = `SELECT orderid FROM o_order WHERE orderman = '${name}' AND state = '0'` 
